...
def setup_model_and_optimizer(
    ...
    if lora_enabled:
        if tp_size > 1:
            assert not lora_cfg.get("use_triton", False), (
    ...
    # Enable input gradients for activation checkpointing with frozen embeddings (e.g. LoRA)
    if config["dtensor_cfg"]["activation_checkpointing"]:
        if hasattr(model, "enable_input_require_grads"):
            model.enable_input_require_grads()
        else:
            print(
                "[WARNING] Model does not support enable_input_require_grads, activation checkpointing might fail with frozen embeddings."
            )
