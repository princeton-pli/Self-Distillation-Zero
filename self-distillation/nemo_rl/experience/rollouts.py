...
def run_multi_turn_rollout(
    ...
    for turn in range(max_rollout_turns):
        ...
        # Calculate max_new_tokens for each sample
        max_new_tokens_list = []
        for length in active_input_lengths:
            max_new_tokens_list.append(max(1, max_seq_len - length.item()))
        generation_input_data["max_new_tokens"] = max_new_tokens_list
    ...
    generation_input_data = BatchedDataDict[GenerationDatumSpec](
        {
            ...
            "max_new_tokens": [max(1, max_seq_len - input_lengths.item())],
        }
    )
...
async def run_sample_multi_turn_rollout(
    ...
    print(f"DEBUG: max_seq_len = {max_seq_len}")
