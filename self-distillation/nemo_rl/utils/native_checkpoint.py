...
# Add Automodel to path
current_dir = os.path.dirname(os.path.abspath(__file__))
nemo_rl_root = os.path.dirname(os.path.dirname(current_dir))
automodel_path = os.path.join(nemo_rl_root, "3rdparty", "Automodel-workspace", "Automodel")
if os.path.exists(automodel_path) and automodel_path not in sys.path:
    sys.path.append(automodel_path)

try:
    from nemo_automodel.components.checkpoint._backports.consolidate_hf_safetensors import consolidate_safetensors_files
except ImportError:
    consolidate_safetensors_files = None
...
def convert_dcp_to_hf(
    ...
    metadata_path = os.path.join(dcp_ckpt_path, ".hf_metadata")
    if not os.path.exists(metadata_path):
        model_subdir = os.path.join(dcp_ckpt_path, "model")
        model_metadata_path = os.path.join(model_subdir, ".hf_metadata")
    ...
    # Check if we should use consolidate_safetensors_files
    # We use it if .metadata (DCP file) is MISSING
    # But we might have .hf_metadata (dir) OR .metadata (dir with HF files)
    
    metadata_file_path = os.path.join(dcp_ckpt_path, ".metadata")
    hf_metadata_dir_path = os.path.join(dcp_ckpt_path, ".hf_metadata")
    
    # Check if standard DCP metadata file exists
    is_dcp = os.path.isfile(metadata_file_path)
    
    # Check if HF metadata dir exists (either as .hf_metadata or .metadata)
    hf_mapping_path = None
    if os.path.isdir(hf_metadata_dir_path):
        hf_mapping_path = os.path.join(hf_metadata_dir_path, "fqn_to_file_index_mapping.json")
    elif os.path.isdir(metadata_file_path):
        hf_mapping_path = os.path.join(metadata_file_path, "fqn_to_file_index_mapping.json")
        
    has_hf_mapping = hf_mapping_path and os.path.exists(hf_mapping_path)

    if not is_dcp and has_hf_mapping and consolidate_safetensors_files is not None:
        print("Detected sharded safetensors checkpoint without DCP metadata. Using consolidate_safetensors_files.")
        
        # Load mapping
        fqn_to_index_mapping = None
        with open(hf_mapping_path, "r") as f:
            fqn_to_index_mapping = json.load(f)
        
        consolidate_safetensors_files(
            input_dir=dcp_ckpt_path,
            output_dir=hf_ckpt_path,
