...
try:
    ...
    # When imported from outside evaluation directory, use relative import
    try:
        from .utils import *
    except ImportError:
        # If still fails, utils functions may not be needed for basic functionality
        pass
...
def extract_answer(pred_str, data_name, use_last_number=True):
    ...
    if data_name in ["mmlu_stem", "sat_math", "aqua", "gaokao2023", "mmlu_pro"]:
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    if data_name in ["math", "minerva_math", "math-skill"]:
        ...
        gt_cot, gt_ans = None, example["answer"]
    elif data_name == "amobench":
        gt_cot, gt_ans = example["solution"], extract_answer(example["answer"], data_name)
    else:
